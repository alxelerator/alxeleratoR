...
#' @param col Color for the residual lines. Default "black".
#' @return Base R plot
#' @note Uses the data from procrustes vegan package
#' @export
#' @author Alex Bossers (UU)

procrustes_error<- function(df="", proc_df, main="Procrustes residuals", col="black"){

  res     <- residuals(proc_df)
  qn      <- quantile(res)
  plot(res, type='h', col=col, lwd=2,
